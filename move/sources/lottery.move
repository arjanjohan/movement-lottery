module lottery_addr::Lottery {
    ...
    #[randomness]
    entry fun pickWinner(acc: &signer) acquires Lotts {
        let addr = signer::address_of(acc);
        let b_store = borrow_global_mut<Lotts>(addr);
        let total_players = vector::length(&b_store.players);
        ...
        let winner_idx = randomness::u64_range(0, total_players);
    ...
    fun test_flow(admin: signer, aptos_framework: &signer) acquires Lotts {
        ...
        let bet5 = account::create_account_for_test(@0x7); // ATLEAST 3 BETTERS
        ...
        let balance = getBalance(&admin);

        assert!(totalPlayers == 5, 0);
        assert!(balance == 14, 0);

        pickWinner(&admin); // ATLEAST 3 BETTERS REQUIRED
